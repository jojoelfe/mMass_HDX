...
class panelSequence(wx.MiniFrame):
    ...
    def onMatchSignal(self, evt=None):
        ...
        reportHTML = generate_fragment_report(self.currentFragments,
                                              self.parent.documents,
                                              self.currentSequence)
        reportFile = file(reportPath, 'w')
        reportFile.write(reportHTML.encode("utf-8"))
        reportFile.close()
        for i, doc_obj in enumerate(self.parent.documents):
            filename = 'data' + str(i) + '.js'
            filePath = os.path.join(tmpDir, filename)
            doc_obj.savejson(filePath,i)
        # show report
