...
m_arrayd *signal_average( m_scanlist *p_scanlist )
{
    ...
    for ( i = 0; i < p_scanlist->numscan; ++i) {
        ...
        p_nextI[i] = j;
        p_nextX[i] = p_scanlist->data[i*2*p_scanlist->scanlength+j*2];
    }
    ...
    while(run) {
        ...
        for ( i = 0; i < p_scanlist->numscan; ++i) {
            ...
            if (p_scanlist->data[i*2*p_scanlist->scanlength+p_nextI[i]*2] == smallest_x) {
                ...
                p_nextX[i] = p_scanlist->data[i*2*p_scanlist->scanlength+p_nextI[i]*2];
                if ( p_nextX[i] == 0 ) {
                    run = false;
                }
            } else { // Otherwise add interpolation
                x1 = p_scanlist->data[i*2*p_scanlist->scanlength+p_nextI[i]*2-2];
...
m_scanlist *scanlist_py2md( PyArrayObject *p_inarr )
{
    m_scanlist *p_outarr;
    int len, numscan, scanlength;
...
static PyObject *_wrap_signal_average( PyObject *self, PyObject *args )
{
    ...
    return PyArray_Return(p_result);
}
...
static PyMethodDef calculations_methods[] = {
   ...
   {"signal_average", _wrap_signal_average, METH_VARARGS, "signal_average( PyArray )"},
